...
#import "JigsawEasyViewController.h"

@interface JigsawEasyViewController ()

@end

@implementation JigsawEasyViewController
...
/*
 Switch that indicates where a new game has been started. This is so that a new game
...
#define kNumPieces 12 //Constant value that represents the number of puzzle pieces, 12.
...
- (void)viewDidLoad
{
    ...
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"puzzlePieceCenterCoordinatesEasy" ofType:@"plist"];
    ...
    [self drawRectangle];
    [self.completeJigsawA setHidden:NO];
...
- (void) setUpNewGame
{
    ...
    [self.completeJigsawA setHidden:YES];
    [self.evaluationLabel setHidden:YES];
    [self generateRandomPositions];
    [self startTimer];
}

// Handles the action for the difficulty select on the UIActionSheet
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0)
    {
        ...
        newGameStarted = FALSE;
        UIStoryboard *jigsawPuzzleBoard = [UIStoryboard storyboardWithName:@"JigsawPuzzleHard" bundle:nil];
...
- (void) endGame
{
    ...
    /*
     ...
     >3 minutes: Slow
     2 - 3 minutes: Satisfactory
     41 seconds - 2 minutes: Good
     20 seconds - 40 seconds: Excellent
     <20 seconds: Outstanding
     */
    if (timeMin >= 3 && timeSec > 0)
    {
    ...
    else if (timeHour == 0 && timeMin >= 0)
    {
        ...
        else if(timeMin == 0 && timeSec >= 20 && timeSec <= 40)
        {
    ...
    gameEnded = TRUE;
    newGameStarted = FALSE;
    [self changeSectionData:@"green-red"];
}
...
-(void)changeSectionData:(NSString *)imageName
{
    ...
    if ([[sections valueForKey:@"Jigsaw Puzzle"] isEqualToString:@"red-green"])
    {
        imageName = @"green-dot";
    }
...
/*
 Method creates a rectangular black border in the middle of the screen to represent
 the border where the puzzle has to be solved in.
 */
-(void)drawRectangle
{
    ...
    self.backgroundRectA = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, height, width)];
    ...
    [self.backgroundRectA.image drawInRect:CGRectMake(0, 0, width, height)];
    ...
    self.backgroundRectA.image = UIGraphicsGetImageFromCurrentImageContext();
   
    // Remove the current bitmap-based graphics context from the top of the stack.
    UIGraphicsEndImageContext();
   
    // Set the background canvas center
    [self.backgroundRectA setCenter:CGPointMake(halfHeight, halfWidth)];
   
    [self.view addSubview:self.backgroundRectA];
    [self.view sendSubviewToBack:self.backgroundRectA];
}
